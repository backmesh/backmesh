...
const proxyInit = JSON.stringify({
	...JSON.parse(anthropicProxyInit),
	allowedPaths: ['v1/messages/count_tokens'],
});
...
describe('Firebase + Anthropic API Proxy Completion usage', () => {
  let response, proxyId, reqHeader: string;
	...
	it('token cost endpoint', async () => {
		response = await SELF.fetch(
			`https://example.com/v1/proxy/${testUserId}/${proxyId!}/v1/messages/count_tokens`,
			{
				method: 'POST',
				headers: {
	...
	it('forbid any endpoint outside of user defined whitelist', async () => {
		response = await SELF.fetch(
